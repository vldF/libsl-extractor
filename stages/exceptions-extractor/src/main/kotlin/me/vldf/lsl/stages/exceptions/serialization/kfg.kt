...
fun getKfgSerialModule(cm: ClassManager, ctx: NameMapperContext): SerializersModule {
    return SerializersModule {
        ...
        contextual(org.vorpal.research.kfg.ir.Class::class, klassSerializer)
        ...
        polymorphic(Type::class) {
            subclass(ClassType::class, ClassTypeSerializer(cm, klassSerializer))
            defaultDeserializer { TypeDefaultSerializer(cm) }
            polymorphicDefaultSerializer(Type::class) { TypeDefaultSerializer(cm) }
        }
    }
}
...
@ExperimentalSerializationApi
@Serializer(forClass = org.vorpal.research.kfg.ir.Class::class)
internal class ClassSerializer(val cm: ClassManager) : KSerializer<org.vorpal.research.kfg.ir.Class> {

    override val descriptor = PrimitiveSerialDescriptor("fullname", PrimitiveKind.STRING)

    override fun serialize(encoder: Encoder, value: org.vorpal.research.kfg.ir.Class) {
...
@ExperimentalSerializationApi
internal inline fun <reified T : org.vorpal.research.kfg.ir.Class> ClassSerializer.to() = object : KSerializer<T> {
    override val descriptor get() = this@to.descriptor
...
internal class MethodSerializer(val cm: ClassManager, val classSerializer: KSerializer<org.vorpal.research.kfg.ir.Class>) :
    KSerializer<Method> {
    ...
    override fun deserialize(decoder: Decoder): Method {
        val input = decoder.beginStructure(descriptor)
        lateinit var klass: org.vorpal.research.kfg.ir.Class
        ...
        input.endStructure(descriptor)
        return klass.getMethod(name, retval, *argTypes)
    }
}
...
internal class InstructionSerializer(
    ...
    override fun serialize(encoder: Encoder, value: Instruction) {
        val output = encoder.beginStructure(descriptor)
        output.encodeSerializableElement(descriptor, 0, methodSerializer, value.parent.parent.method)
        output.encodeStringElement(descriptor, 1, "${value.name}")
        output.endStructure(descriptor)
    }
...
@ExperimentalSerializationApi
internal class TypeDefaultSerializer(val cm: ClassManager) : KSerializer<Type> {

    override val descriptor = buildClassSerialDescriptor("Type") {
        element<String>("typeName")
    }

    override fun serialize(encoder: Encoder, value: Type) = encoder.encodeStructure(descriptor) {
        encodeStringElement(descriptor, 0, value::class.java.name)
    }

    override fun deserialize(decoder: Decoder): Type = decoder.decodeStructure(descriptor) {
        var klassName = ""
        while (true) {
            when (val index = decodeElementIndex(descriptor)) {
                0 -> klassName = decodeStringElement(descriptor, 0)
                CompositeDecoder.DECODE_DONE -> break
                else -> error("Unexpected index: $index")
            }
        }
        val klass: KClass<Type> = Class.forName(klassName).kotlin as KClass<Type>
        val objInstance = klass.objectInstance
        if (objInstance != null) return@decodeStructure objInstance
        error("Unsupported type $klassName")
    }
}


@ExperimentalSerializationApi
internal class ClassTypeSerializer(
    private val cm: ClassManager,
    private val classSerializer: KSerializer<org.vorpal.research.kfg.ir.Class>
) : KSerializer<ClassType> {
    override val descriptor = buildClassSerialDescriptor("ClassType") {
        element("class", classSerializer.descriptor)
    }

    override fun serialize(encoder: Encoder, value: ClassType) = encoder.encodeStructure(descriptor) {
        encodeSerializableElement(descriptor, 0, classSerializer, value.klass)
    }

    override fun deserialize(decoder: Decoder): ClassType = decoder.decodeStructure(descriptor) {
        var klass: org.vorpal.research.kfg.ir.Class? = null
        while (true) {
            when (val index = decodeElementIndex(descriptor)) {
                0 -> klass = decodeSerializableElement(descriptor, 0, classSerializer)
                CompositeDecoder.DECODE_DONE -> break
                else -> error("Unexpected index: $index")
            }
        }
        ClassType(klass ?: error("ClassType deserialization error"))
    }
}
